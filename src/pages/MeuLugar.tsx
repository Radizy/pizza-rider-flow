...
export default function MeuLugar() {
  ...
  const [unidade, setUnidade] = useState<Unidade>('ITAQUA');
  const [searchTelefone, setSearchTelefone] = useState('');
  const [searchUnidade, setSearchUnidade] = useState<Unidade>('ITAQUA');

  const { data: positionData, isLoading, isError } = useQuery({
    queryKey: ['motoboy-position', searchTelefone, searchUnidade],
    queryFn: () => getMotoboyPosition(searchTelefone, searchUnidade),
